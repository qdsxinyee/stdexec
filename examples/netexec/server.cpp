...
auto make_client(auto client) -> exec::task<void> {
    try {
        char buffer[8];
        while (auto size = co_await net::ip::tcp::async_receive(client, net::buffer(buffer))) {
            std::string_view message(buffer, size);
            std::cout << "received<" << size << ">(" << message << ")\n";
            auto ssize = co_await net::ip::tcp::async_send(client, net::const_buffer(buffer, size));
    ...
    try {
        ...
        ex::spawn(
            [](netexec::scope& scp) -> exec::task<void> {
                ...
                net::ip::tcp::acceptor acceptor(scp.get_context(), endpoint.protocol());
