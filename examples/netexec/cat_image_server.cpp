...
auto cat_image_client(auto client) -> exec::task<void> {
    try {
        ...
        co_await net::ip::tcp::async_send(client, net::buffer(response_str));
...
auto run_server(net::io_context& ctx, stdexec::counting_scope::token token) -> exec::task<void> {
    ...
    net::ip::tcp::acceptor server(ctx, endpoint.protocol());
    co_await net::ip::tcp::async_listen(server, endpoint);
