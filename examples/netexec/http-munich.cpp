...
auto run_client(auto stream) -> exec::task<void> {
    ...
    char buffer[1000];
    while (std::size_t n = co_await net::ip::tcp::async_receive(stream, net::buffer(buffer))) {
        ...
        if ((in >> method >> url >> version) && method == "GET" && files.contains(url)) {
            ...
            co_await net::ip::tcp::async_send(stream, net::buffer(response));
        }
    }
...
auto main() -> int {
    ...
    net::ip::tcp::acceptor server(scope.get_context(), ep.protocol());
    server.bind(ep);
    server.listen();
