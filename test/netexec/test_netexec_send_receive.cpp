...
auto echo_client(netexec::scope& scope, std::uint16_t port, std::string* response) -> exec::task<void> {
    ...
    co_await net::ip::tcp::async_send(socket, net::const_buffer("hello", 5));

    char buffer[64];
    auto n = co_await net::ip::tcp::async_receive(socket, net::buffer(buffer));
...
auto multiple_send_client(netexec::scope& scope, std::uint16_t port, std::string* response) -> exec::task<void> {
    ...
    co_await net::ip::tcp::async_send(socket, net::const_buffer("abc", 3));
    co_await net::ip::tcp::async_send(socket, net::const_buffer("def", 3));
    co_await net::ip::tcp::async_send(socket, net::const_buffer("ghi", 3));
...
auto receive_until_close(netexec::scope& scope, std::uint16_t port, std::size_t* received) -> exec::task<void> {
    ...
    char buffer[64];
    *received = co_await net::ip::tcp::async_receive(socket, net::buffer(buffer));
}
