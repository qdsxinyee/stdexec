...
auto run_acceptor(netexec::scope& scope, std::uint16_t port, bool* accepted) -> exec::task<void> {
    ...
    co_await net::ip::tcp::async_connect(client);
    auto [server, server_ep] = co_await net::ip::tcp::async_accept(acceptor);
    *accepted = true;
    (void)server_ep;
}
...
TEST_CASE("netexec - acceptor is move constructible", "[netexec][acceptor]") {
    netexec::scope scope;
    auto ep = netexec_test::make_server_endpoint(netexec_test::next_port());
    net::ip::tcp::acceptor a(scope.get_context(), ep.protocol());
    a.bind(ep);
    a.listen();
