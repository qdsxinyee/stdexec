...
auto client_send(netexec::scope& scope, std::uint16_t port, int index) -> exec::task<void> {
    net::ip::tcp::socket socket(scope.get_context(), netexec_test::make_server_endpoint(port));
    auto ok = co_await (net::ip::tcp::async_connect(socket)
                        | ex::then([] { return true; })
                        | ex::upon_error([](auto&&) { return false; }));
    CHECK(ok);

    char c = static_cast<char>('a' + index);
    co_await net::ip::tcp::async_send_some(socket, net::const_buffer(&c, 1));
}
