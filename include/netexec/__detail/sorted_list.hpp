...
struct netexec::detail::sorted_list {
...
template <typename T, typename Compare, typename Value>
auto netexec::detail::sorted_list<T, Compare, Value>::pop_front() -> T* {
    next_t rc{this->head};
    this->head = rc->next;
...
template <typename T, typename Compare, typename Value>
auto netexec::detail::sorted_list<T, Compare, Value>::insert(T* node) -> T* {
    next_t* it{&this->head};
...
template <typename T, typename Compare, typename Value>
auto netexec::detail::sorted_list<T, Compare, Value>::erase(next_t node) -> next_t {
    next_t* it{&this->head};
