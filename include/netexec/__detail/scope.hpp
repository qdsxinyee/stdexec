...
class netexec::detail::scope {
  public:
    ...
    class token {
      public:
        struct work_association {
            stdexec::counting_scope::__assoc_t inner;
            scope*                               _scope{};

            work_association() = default;
            work_association(stdexec::counting_scope::__assoc_t a, scope* s)
                : inner(::std::move(a)), _scope(s) {}
            work_association(work_association&&)                    = default;
            work_association& operator=(work_association&&) = default;

            explicit operator bool() const noexcept { return static_cast<bool>(this->inner); }

            auto try_associate() const noexcept -> work_association {
                if (!*this) {
                    return work_association{};
                }
                work_association nested{this->inner.try_associate(), this->_scope};
                if (nested) {
                    this->_scope->_io_context.add_work();
                }
                return nested;
            }
            ...
            ~work_association() {
                if (*this) {
                    this->_scope->_io_context.remove_work();
                }
            }
        };
        ...
        auto try_associate() const noexcept -> work_association {
            work_association assoc{this->_counting_token.try_associate(), this->_scope};
            if (assoc) {
                this->_scope->_io_context.add_work();
            }
            return assoc;
